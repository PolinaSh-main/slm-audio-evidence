...
| 3 | Task & Diagnostic Set (1.5 p) [MIN] | M2 → M3 | task definition (answer or abstain); A/B/C + C1–C4 taxonomy; pilot-100 construction, 80% agreement, freeze; AQUA-Bench conversion procedure |
...
| 5 | Experiments & Results (2–2.5 p) | M2+M4 (+M1: probe tables) → M1 | pilot table (4 runs, Wilson CIs) [MIN]; AQUA validation table [MIN]; AUROC table probes vs entropy [A2]; operating-points plot probe→abstain vs plain/S1 [A2] |
