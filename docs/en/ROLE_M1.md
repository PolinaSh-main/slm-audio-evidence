...
2. **Decision 1 — AQUA format (Mon Jul 20, ~30 min).** M2 proposes: keep MCQ / convert to free-form / both. You pick one, write it to [../decisions.md](../decisions.md).
3. **Decision 2 — probe design (Wed Jul 22, ~1 h).** After reading paper 24, log in decisions.md: which layers, which pooling, k for soft targets, the cross-validation protocol (agree with M4). This is the design record for the team and the hand-over insurance.
4. **The A2 core — your main part (Jul 22–29).** You write the representation-capture code (M2 runs it on DataSphere, tensors by Jul 23; the k≈10 sampling is also M2's runs, labels from M4 by Jul 24), train linear probes per layer (logreg + CV per M4's protocol), build the AUROC table for the checkpoint, then implement the attention probe (runs and debugging with M2) and do the mitigation analysis (operating points vs plain / S1 / entropy). Insurance: if you are unavailable — M2 trains the linear probes from the design record of step 3.
5. **Decision 3 — checkpoint (Fri Jul 24, ~1 h).** Look at the AQUA table + your own first AUROC. Rule: no working AUROC → switch to plan-minimum (PLAN §5) and announce it. Continuing A2 without numbers is not allowed.
...
| 01 — AQUA-Bench | Decision 1 | skim: format, categories |
