...
{"id": "sq-0421-C2",               // unique: source–passage–category+number
 ...
 "source": "spoken-squad",         // spoken-squad | tts | aqua-bench
 "generator": "gpt-4o-mini/b-v2",  // model/prompt-version; "native" for original questions
...
**A1 · AQUA-Bench validation (M2 runs, M4 judges).** Does our pilot finding hold on an external benchmark? Steps: get AQUA-Bench data (paper 01) → convert a speech-relevant subset (~200–400 items) into our manifest schema → run **both systems** (Qwen2-Audio and cascade) × {plain, S1 IDK} → grade with M4's LLM-judge → one table: hallucination / correct-abstain / accuracy / over-refusal, pilot vs AQUA side by side. Design decision on Jul 20 (M2 proposes, M1 decides): keep their multiple-choice format, convert to free-form, or both.
...
*On strategies S2–S5 from the pre-defense plan:* in the paper phase we deliberately run only plain and S1. Our comparison axis is "prompt (S1) vs output-level signal (entropy) vs pre-generation signal (probe)", and a broad prompt-strategy survey already exists in paper 02 — repeating it is not a contribution. S4 is not dropped but moved *inside* the method (the same k samples give soft targets and entropy); S5 is an optional third baseline (step 4); S2 may return only as the MCQ format in AQUA (Decision 1). S3 (self-verification) is out of the paper phase.
...
**Checkpoint Fri Jul 24 (M1 decides): AQUA numbers exist + first probe AUROC exists?** → continue A2, or switch to plan-minimum (§5).
...
| Jul 23–29 | Task & Dataset section (pilot-100 as *diagnostic set* + AQUA conversion) | M2 |
...
pilot-100 diagnostic set + AQUA-Bench validation + LLM-judge with 94% human agreement + related work + honest limitations; probing becomes Future Work. This is still a complete, defensible workshop-grade paper. **No dataset expansion in either scenario** — pilot-100 stays frozen as the diagnostic set.
...
| Who | Papers | By | Focus |
|---|---|---|---|
| **Everyone** | 24 (probing — the method we transfer), 01 (AQUA-Bench) | Tue Jul 21 | 24: method + results; 01: data format, unanswerable categories |
...
| Mon–Tue 20–21 | A1 format decision | A1 convert + first runs | reading + bullets | **judge rework + new judge models (Jul 21)** |
| Wed–Thu 22–23 | reading done; probe design (Decision 2); capture code | A2: capture run with M1's code (tensors by the 23rd) | reading + bullets; **Intro/RW draft starts (23rd)** | judge on A1 outputs; grading samples for soft targets |
...
| **Wed Jul 29** | **stop experiments**; final probe tables | final A1 tables | draft polish | Methods section starts |
...
- 100 items → wide Wilson confidence intervals; we report them and add AQUA scale — never hide them.
