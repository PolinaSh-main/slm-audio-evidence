...
- English only.
- Small pilot scale: 40 passages and a final target of 90-120 verified items.
- Source domain is SQuAD-style Wikipedia passages, so topic and style diversity are limited.
- Audio is from Spoken-SQuAD rather than newly recorded natural conversations; TTS fallback is available but should be documented if used.
- B/C questions are generated by a chat model and require human verification before freeze.
- Current agreement number is `TBD` until double verification is completed.
