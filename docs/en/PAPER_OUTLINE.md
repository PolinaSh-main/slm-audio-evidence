...
| 3 | Task & Diagnostic Set (1.5 p) [MIN] | M2 → M3 | task definition (answer or abstain); A/B/C + C1–C4 taxonomy; pilot-100 construction, 80% agreement, freeze; scale-set construction: native SQuAD 2.0 questions over Spoken-SQuAD audio + the natural NMSQA slice |
...
| 5 | Experiments & Results (2–2.5 p) | M2+M4 (+M1: probe tables) → M1 | pilot table (4 runs, Wilson CIs) [MIN]; scale-set validation table (native SQuAD 2.0; TTS vs natural speech if the NMSQA slice landed) [MIN]; AUROC table probes vs entropy [A2]; operating-points plot probe→abstain vs plain/S1 [A2] |
| 6 | Discussion (0.5–1 p) [MIN] | M1 | what the cascade gap means; why pre-generation matters; two-team landscape |
| 7 | Limitations (0.5 p) [MIN] | M1 | 100 items/wide CIs; one model family; LLM judge; English-only; TTS-heavy audio |
